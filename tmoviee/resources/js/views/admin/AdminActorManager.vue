    ...
    <div class="flex-1 p-4 min-h-screen">
      <div class="p-6 pt-header min-h-screen">
        <div class="flex justify-between items-center mb-6">
          <el-input v-model="search" placeholder="Tìm kiếm diễn viên..." style="max-width: 300px" @keyup.enter="onSearch" clearable @clear="onSearch" class="shadow-sm" />
          <button @click="openAddModal" class="bg-yellow-400 hover:bg-yellow-300 text-gray-900 font-bold px-5 py-2 rounded-lg shadow flex items-center">
            <i class="fas fa-plus mr-2"></i> Thêm diễn viên mới
          </button>
        </div>
        <el-table :data="actors" style="width: 100%" v-loading="loading" class="shadow-md rounded-lg overflow-hidden bg-[#23262f] text-white">
        ...
        />
        <el-dialog :title="editActor ? 'Sửa diễn viên' : 'Thêm diễn viên'" v-model="modalVisible" width="500px" :close-on-click-modal="false" class="!bg-[#23262f] rounded-xl shadow-xl p-8 overflow-y-auto max-h-[90vh] hide-scrollbar text-white">
          ...
          <template #footer>
            ...
            <el-button type="primary" @click="submitForm" class="shadow-sm">{{ editActor ? 'Cập nhật' : 'Tạo mới' }}</el-button>
          </template>
        </el-dialog>
      </div>
    </div>
...
function submitForm() {
  formRef.value.validate(async valid => {
    ...
    try {
      if (editActor.value) {
        await axios.put(`/api/admin/actors/${editActor.value.id}`, form.value);
        ElMessage.success('Cập nhật thành công');
      } else {
        await axios.post('/api/admin/actors', form.value);
...
function confirmDelete(row) {
  ElMessageBox.confirm('Bạn chắc chắn muốn xóa diễn viên này?', 'Xác nhận', {
    type: 'warning',
    confirmButtonText: 'Xóa',
    cancelButtonText: 'Hủy',
  }).then(async () => {
    await axios.delete(`/api/admin/actors/${row.id}`);
    ElMessage.success('Đã xóa diễn viên');
    fetchActors();
  });
}
